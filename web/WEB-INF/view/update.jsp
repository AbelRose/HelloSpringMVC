...
<body>
	...
	<form action="${pageContext.servletContext.contextPath }/emp" method="post"> <!-- 需要是绝对路径 -->
		<input type="hidden" name="id" value="${emp.id }" /> <!-- 隐藏域ID -->
				...
				<td>
					<select name="department.id">
						...
						<c:forEach items="${depts }" var="dept">
							<option value="${dept.id }" ${dept.id == emp.department.id?'selected':'' }>${dept.departmentName }</option> <!-- 在下拉框中 ?'selected':''用于回显  -->
						</c:forEach>
					</select>
				</td>
			</tr>
